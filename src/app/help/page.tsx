  ...
  const pages: Array<[string, string, string[]]> = [
    ...
    [
      ...
      [
        ...
        'A project with phases shows how far it has got, as a bar and as a percentage. A phase you have ticked counts for a whole one; a phase still open counts for the share of its own tasks that are done, so work inside a phase moves the project bar before the phase itself is ticked. Every phase weighs the same, whether it holds one task or twenty. 100% means every phase is ticked \u2014 a project whose work is finished but not yet ticked off stops just short, because the tick is yours to make. One without phases shows neither bar nor number, because there is nothing to be a fraction of.',
        'A task on a project can be filed under one of its phases — from the task’s edit dialog, or typed straight into the phase’s own block on the project page. A filed task sits inside its phase, and the phase gets its own bar, counted from its tasks — it fills as they are done, while ticking the phase itself stays yours. Tasks under no phase sit in Uncategorized tasks.',
        'A phase can be marked “only inside this project”, in its pencil dialog. Its tasks then show nowhere but this page — not on Tasks, not on the Overview — which is for phases whose work is planned rather than actionable yet. The phase wears an “Only in this project” mark beside its name, so a task that seems missing has a visible reason.',
